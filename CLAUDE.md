...
**The address book** (`/api/contacts`, `services/addressBook.ts`, page `/contacts`) lists every contact of the account across its campaigns, searchable, sortable from a fixed map of `ORDER BY` clauses (never a column name from the request), paginated. It is a view, not a second store: a contact still belongs to one campaign. Adding and editing are allowed only while that campaign is a draft, checked inside the SQL write itself; removing is always allowed, the send log keeping its line with the contact detached. `contacts.source` (`csv`, `manual`, `mailfind`) says where a contact came from; a follow-up copies it. A selection of the address book becomes a new draft through `POST /api/campaigns/follow-up` (with its type); `POST /api/contacts/copy` loads existing contacts into a draft, once per address, the owner checked in the SQL; `GET /api/contacts/export` is the CSV of everything the filters match. The picker in a campaign lists with `unique=true` (the most recent contact per address) and `exclude_campaign_id` (what the draft already holds).
