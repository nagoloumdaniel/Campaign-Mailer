...
- **Office hours only: Monday to Saturday, 09:00 to 18:59**, on the campaign's own clock. The owner set 10:00 to 17:59 every day on 16 September 2026, then moved it on 22 September 2026 to the current window, Sundays closed. It is about how the message is received rather than about Gmail: a candidature landing at three in the morning, or on a Sunday, reads as automated. `FIRST_SEND_HOUR`, `LAST_SEND_HOUR` and `isSendDay` live in `services/planner.ts`, mirrored in `schemas/campaign.ts` and `frontend/src/services/campaigns.ts`; `nextOpening` in `services/campaignStats.ts` finds the next Monday-to-Saturday 09:00. The planner stops a day's plan where the window closes, so 450 messages thirty seconds apart do not deliver into the night; what does not fit waits for the next opening.
- **Three settings are the application's, not the user's** (owner's decision, 22 September 2026): the start hour is the opening of the window (a CHECK holds `start_hour = 9`); the pause between two sends is thirty seconds plus jitter, an anti-spam measure; the time zone is the browser's, sent at creation and at launch. The API refuses `start_hour` and `pause_ms` in a payload. Only the daily pace, `mails_per_day`, stays a setting. Do not bring the others back as fields.
- **A launch may be scheduled.** `POST /campaigns/:id/start` takes an optional `send_after` (ISO with offset, within 90 days, inside the window on the campaign's clock, else 400 `outside_send_window`) and the browser's `timezone`. It is stored in `campaigns.send_after`, distinct from `scheduled_at` (when launch was pressed). The dispatcher returns `scheduled_later` with `retryAt = send_after` until then, so the campaign stays `scheduled` and the worker plans at that second.
- **The campaign saves itself.** The editor saves the message 1.2 s after the last keystroke and the pace 0.8 s after it; there is no save button. A save that finishes while the user kept typing leaves the draft marked dirty so the next pause saves the rest.
...
**The address book** (`address_book`, `services/addressBook.ts`, `/api/contacts`, page `/contacts`) holds one entry per email address and per account (unique on `user_id, lower(email)`), whatever brought it: CSV, manual add, follow-up, MailFind. Owner's design of 22 September 2026. Campaign recipients (`contacts`) are a snapshot linked by `contacts.book_id`; the `contacts_link_address_book` trigger, BEFORE INSERT on `contacts`, creates the entry for an unknown address and, for a known one, keeps the entry's values and fills only its empty fields (owner's choice), while the recipient borrows from the entry what the import left empty. Editing an entry rewrites only recipients still `pending` or `ignored`; a sent or failed recipient keeps what went out, so the history never changes. Removing an entry deletes its recipients not yet sent to and with no log row (deleting a recipient cascades to its logs), and leaves the others unlinked. The page shows only name, address, company, salutation, date added and origin; the add and edit dialog is the campaign's `AddContactDialog`, same four fields. A selection becomes a draft through `POST /api/campaigns` then `POST /api/contacts/copy`; a draft's "Mes contacts" picker lists the book with `exclude_campaign_id`. The daily backup includes `address_book`, before `contacts`.
...
- **The end-to-end run pins the planner's clock to noon UTC** (`src/e2e/server.ts`), Saturday's when run on a Sunday. Sending only happens Monday to Saturday, 09:00 to 18:59 on the campaign's clock, and the suite runs whenever somebody runs it; without the pin a journey started at midnight or on a Sunday would plan nothing. The same reasoning put a `noonUtc()` default in `jobs/pipeline.integration.test.ts`. Only the clock is pinned — which contacts are picked, how many, and the ceiling they are checked against are the real rules.
